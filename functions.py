...
# Copy the files
def copy_files(output, input_path=INPUT_DATA_PATH, output_path=SPLIT_DATA_PATH):
    list_of_sets = ["train", "validation", "test"]
    for i, out in enumerate(output):
        for file_name in out:
            shutil.copy(f"{input_path}/{file_name}.jpg", f"{output_path}/{list_of_sets[i]}/images/{file_name}.jpg")
            shutil.copy(f"{input_path}/{file_name}.txt", f"{output_path}/{list_of_sets[i]}/labels/{file_name}.txt")
